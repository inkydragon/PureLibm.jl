...
# Based on core-math/src/binary32/cosh/coshf.c
# CORE-MATH project Copyright (c) 2022-2023 Alexei Sibidanov.

const CR_COSHF_C = NTuple{4, Float64}((
    1, 0x1.62e42fef4c4e7p-6, 0x1.ebfd1b232f475p-13, 0x1.c6b19384ecd93p-20
))
const CR_COSHF_CH = NTuple{7, Float64}((
    1.0, Float64(0x1.62e42fefa39efp-6),
    Float64(0x1.ebfbdff82c58fp-13), Float64(0x1.c6b08d702e0edp-20),
    Float64(0x1.3b2ab6fb92e5ep-27), Float64(0x1.5d886e6d54203p-35), Float64(0x1.430976b8ce6efp-43)
))
const CR_COSHF_TB = NTuple{32, UInt64}((
...
const CR_COSHF_CP = NTuple{4, Float64}((
...
function cr_coshf(x::Float32)
    ...
    if @unlikely(ax > 0x8565_a9f8)
        ...
        if ax >= 0xff00_0000
            ...
            return Float32(Inf)  # Inf
        end
        r = 2.0f0 * Float32(0x1.fffffep127)
        return r
    end
    ...
    if @unlikely(ax < 0x7c00_0000)
        ...
        if @unlikely(ax < 0x7400_0000)
            ...
            return Float32((0.5f0 * x) * x + 1.0f0)
        end

        cp = CR_COSHF_CP
        z2 = z * z
        z4 = z2 * z2
        r = 1.0 + z2 * ((cp[1] + z2 * cp[2]) + z4 * (cp[3] + z2 * cp[4]))
        return Float32(r)
    end
    ...
    jau = reinterpret(UInt64, Float64(ia + 0x1.8p52))
    jp = Int64(jau)
    jm = -jp

    tb = CR_COSHF_TB
    spu = tb[(jp & 31) + 1] + (reinterpret(UInt64, jp >> 5) << 52)
    smu = tb[(jm & 31) + 1] + (reinterpret(UInt64, jm >> 5) << 52)
    spf = reinterpret(Float64, spu)
    smf = reinterpret(Float64, smu)
    ...
    if @unlikely(ub != lb)
        ...
        h2 = h * h
        ch = CR_COSHF_CH
        te = ch[1] + h2 * ch[3] + (h2 * h2) * (ch[5] + h2 * ch[7])
        to = ch[2] + h2 * (ch[4] + h2 * ch[6])

        r = spf * (te + h * to) + smf * (te - h * to)
        ub = Float32(r)
    end

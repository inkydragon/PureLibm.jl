...
# Based on core-math/src/binary32/log2/log2f.c
# CORE-MATH project Copyright (c) 2022 Alexei Sibidanov.

#! format: off
# Keep same format as in log2f.c
const CR_LOG2F_IX = Vector{Float64}([
...
const CR_LOG2F_LIX = Vector{Float64}([
    0x0p+0,
...
function cr_log2f(x::Float32)
    ...
    m = UInt64(ux & (~UInt32(0) >> 9))
    m <<= (52 - 23)
    e = Int(ux >> 23) - 0x7f
    if (ux < (UInt32(1) << 23) || ux >= (UInt32(0xff) << 23))
        if ux == 0 || ux == (UInt32(1) << 31)
            # x = +/-0
            # errno = ERANGE
            return -1.0f0 / 0.0f0  # should raise 'Divide by zero' exception.
        end
        inf_or_nan = ((ux >> 23) & 0xff) == 0xff
        nan = inf_or_nan && ((ux << 9) != 0)
        if (ux >> 31) != 0 && !nan
            # x < 0
            # errno = EDOM
    ...
    j = Int((m + (Int64(1) << (52 - 8))) >> (52 - 7))
    k = j > 53
    if k
        e += 1
    end

    xd = reinterpret(Float64, m | (UInt64(0x3ff) << 52))
    z = fma(xd, CR_LOG2F_IX[j+1], -1.0)  # z is exact
    z2 = z * z
    c = CR_LOG2F_C
    c0 = c[1] + z * c[2]
    c2 = c[3] + z * c[4]
    ...
    return Float32((z * iln2) * c0 + (e - CR_LOG2F_LIX[j+1] * iln2))
end

...
function _logf_as_special(x::Float32)::Float32
    ...
    if ux == 0x00000000  # +0.0
        # errno = ERANGE
        return -1.0 / 0.0  # to raise FE_DIVBYZERO
    elseif ux == 0x7f800000  # +inf
        return x
    end
    ...
    if ax == 0x00000000  # -0.0
        # errno = ERANGE
        return -1.0 / 0.0  # to raise FE_DIVBYZERO
    elseif ax > 0xff000000  # nan
        return x + x
    end

    # errno = EDOM
    return 0.0 / 0.0  # to raise FE_INVALID and return nan
end
...
function cr_logf(x::Float32)::Float32
    ...
    if @unlikely(ux == (127 << 23))
        return Float32(0.0)
    end

    m = ux & ((UInt32(1) << 23) - UInt32(1))
    j = (m + (UInt32(1) << (23 - 7))) >> (23 - 6)
    e = (reinterpret(Int32, ux) >> 23) - 127
    tzu = ((UInt64(m) | (Int64(1023) << 23)) << (52 - 23))
    tzf = reinterpret(Float64, tzu)

    z = tzf * CR_LOGF_TR[j+1] - 1
    z2 = z * z
    tl = CR_LOGF_TL
    b = CR_LOGF_B
    r = ((e * 0x1.62e42fefa39efp-1 + tl[j+1]) + z * b[1]) + z2 * (b[2] + z * b[3])
    ub = Float32(r)
    lb = Float32(r + 0x1.f06p-33)

# SPDX-License-Identifier: MIT OR Apache-2.0
# Based on core-math/src/binary32/sinh/sinhf.c
# CORE-MATH project Copyright (c) 2022-2025 Alexei Sibidanov.

const CR_SINHF_CP = NTuple{4,Float64}((
    0x1.5555555555555p-3, 0x1.11111111146e1p-7, 0x1.a01a00930dda6p-13, 0x1.71f92198aa6e9p-19
))
const CR_SINHF_C = NTuple{4,Float64}((
    1.0, 0x1.62e42fef4c4e7p-6, 0x1.ebfd1b232f475p-13, 0x1.c6b19384ecd93p-20
))
const CR_SINHF_CH = NTuple{7,Float64}((
    1.0,
    0x1.62e42fefa39efp-6,
    0x1.ebfbdff82c58fp-13,
    0x1.c6b08d702e0edp-20,
    0x1.3b2ab6fb92e5ep-27,
    0x1.5d886e6d54203p-35,
    0x1.430976b8ce6efp-43,
))
const CR_SINHF_TB = UInt64[
    0x3fe0000000000000,
    0x3fe059b0d3158574,
    0x3fe0b5586cf9890f,
    0x3fe11301d0125b51,
    0x3fe172b83c7d517b,
    0x3fe1d4873168b9aa,
    0x3fe2387a6e756238,
    0x3fe29e9df51fdee1,
    0x3fe306fe0a31b715,
    0x3fe371a7373aa9cb,
    0x3fe3dea64c123422,
    0x3fe44e086061892d,
    0x3fe4bfdad5362a27,
    0x3fe5342b569d4f82,
    0x3fe5ab07dd485429,
    0x3fe6247eb03a5585,
    0x3fe6a09e667f3bcd,
    0x3fe71f75e8ec5f74,
    0x3fe7a11473eb0187,
    0x3fe82589994cce13,
    0x3fe8ace5422aa0db,
    0x3fe93737b0cdc5e5,
    0x3fe9c49182a3f090,
    0x3fea5503b23e255d,
    0x3feae89f995ad3ad,
    0x3feb7f76f2fb5e47,
    0x3fec199bdd85529c,
    0x3fecb720dcef9069,
    0x3fed5818dcfba487,
    0x3fedfc97337b9b5f,
    0x3feea4afa2a490da,
    0x3fef50765b6e4540,
]

"""
Correctly-rounded hyperbolic sine of `Float32`.
"""
function cr_sinhf(x::Float32)
    iln2 = 0x1.71547652b82fep+5

    tu = reinterpret(UInt32, x)
    ux = tu << 1
    z = Float64(x)
    if (ux > 0x8565a9f8)
        # |x| > 0x1.65a9f8p+6
        sgn = copysign(2.0f0, x)
        if (ux >= 0xff000000)
            if (ux << 8) != 0
                return x + x  # NaN
            end
            return sgn * Inf32
        end
        r = sgn * Float32(0x1.fffffep127)
        return r
    end

    if (ux < 0x7c000000)
        # |x| < 0.125
        if (ux <= 0x74250bfe)
            # |x| <= 0x1.250bfep-11
            if (ux < 0x66000000)
                # |x| < 0x1p-24
                return fma(x, abs(x), x)
            end
            if ux == 0x74250bfe
                sgn = copysign(1.0f0, x)
                return sgn * Float32(0x1.250bfep-11) + sgn * Float32(0x1p-36)
            end
            return (x * Float32(0x1.555556p-3)) * (x * x) + x
        end

        z2 = z * z
        z4 = z2 * z2
        cp = CR_SINHF_CP
        r = z + (z2 * z) * ((cp[1] + z2 * cp[2]) + z4 * (cp[3] + z2 * cp[4]))
        return Float32(r)
    end

    a = iln2 * z
    ia = _llvm_roundeven(a)
    h = a - ia
    h2 = h * h

    jaf = ia + 0x1.8p52
    jau = reinterpret(UInt64, jaf)
    jp = reinterpret(Int64, jau)
    jm = -jp

    tb = CR_SINHF_TB
    spu = tb[(jp&31)+1] + (reinterpret(UInt64, jp >> 5) << 52)
    smu = tb[(jm&31)+1] + (reinterpret(UInt64, jm >> 5) << 52)
    spf = reinterpret(Float64, spu)
    smf = reinterpret(Float64, smu)

    c = CR_SINHF_C
    te = c[1] + h2 * c[3]
    to = c[2] + h2 * c[4]

    rp = spf * (te + h * to)
    rm = smf * (te - h * to)
    r = rp - rm

    ub = Float32(r)
    lb = Float32(r - 1.52e-10 * r)
    if (ub != lb)
        iln2h = 0x1.7154765p+5
        iln2l = 0x1.5c17f0bbbe88p-26
        h = (iln2h * z - ia) + iln2l * z
        h2 = h * h
        ch = CR_SINHF_CH
        te = ch[1] + h2 * ch[3] + (h2 * h2) * (ch[5] + h2 * ch[7])
        to = ch[2] + h2 * (ch[4] + h2 * ch[6])
        r = spf * (te + h * to) - smf * (te - h * to)
        ub = Float32(r)
    end

    return ub
end

...
const CR_ASINHF_IX = Vector{Float64}([
...
const CR_ASINHF_LIX = Vector{Float64}([
...
const CR_ASINHF_C1 = NTuple{8, Float64}((
...
const CR_ASINHF_C2 = NTuple{3, Float64}((
    0x1.0000000066947p+0, -0x1.00007f053d8cbp-1, 0x1.555280111d914p-2
))
const CR_ASINHF_CP = NTuple{6, Float64}((
...
function cr_asinhf(x::Float32)
    ...
    if @unlikely(tu <= 0x3e815667)
        ...
        x8 = x4 * x4
        c = CR_ASINHF_C1
        f = x2 * (((c[1] + x2 * c[2]) + x4 * (c[3] + x2 * c[4])) + x8 * ((c[5] + x2 * c[6]) + x4 * (c[7] + x2 * c[8])))
        r = xs - xs * f
        return Float32(r)
    else
        ...
        m = UInt64(tpu & (~UInt64(0) >> 12))
        j = Int64((m + (Int64(1) << (52 - 8))) >> (52 - 7))
        e = Int64((tpu >> 52) - 0x3ff)
        wu = m | (UInt64(0x3ff) << 52)
        wf = reinterpret(Float64, wu)
        z = wf * CR_ASINHF_IX[j+1] - 1.0
        z2 = z * z
        lix = CR_ASINHF_LIX
        c = CR_ASINHF_C2

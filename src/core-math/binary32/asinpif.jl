...
# Based on core-math/src/binary32/asinpi/asinpif.c
# CORE-MATH project Copyright (c) 2023 Alexei Sibidanov.

#! format: off
# Keep same format as in asinpif.c
const CR_ASINPIF_CH = Vector{NTuple{8, Float64}}([
...
function cr_asinpif(x::Float32)
    ...
    if @unlikely(i == 0)
        ...
        c4 += c6 * z2
        c0 += c4 * z4
        r = fma(-c0, copysign(f, x), copysign(0.5, x))
        return Float32(r)
    end
end

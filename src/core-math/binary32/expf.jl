...
function cr_expf(x::Float32)::Float32
    ...
    if @unlikely(ux > 0x8562e42e || ux < 0x6f93813e)
        ...
        if tu > 0xc2ce8ec0  # -103.27881f0
            y = 0x1p-149 + (z + 0x1.9d1d9fccf477p+6) * 0x1.71547652b82edp-150
            y = max(y, 0x1p-151)
            r = Float32(y)
            # if r == 0.0f0
            #     errno = EDOM
    ...
    ia = big - uf
    h = a + ia
    svf = reinterpret(Float64, CR_EXPF_TB[(uu & 0x3f)+1] + ((uu >> 6) << 52))
    h2 = h * h
    b = CR_EXPF_B
    ...
    if @unlikely(ub != lb)
        iln2h = 0x1.7154765p+0
        iln2l = 0x1.5c17f0bbbe88p-31
        h = (iln2h * z + ia) + iln2l * z
        s = svf
        h2 = h * h
        w = s * h
        c = CR_EXPF_C
        r = s + w*((c[1] + h*c[2]) + h2*((c[3] + h*c[4]) + h2*(c[5] + h*c[6])))
        ub = Float32(r)
    end

...
pos_range = (lo=+Float32(0.0), hi=+Float32(39))    # 88.72
neg_range = (lo=-Float32(0.0), hi=-Float32(18))     # -17.32
if "cr_expm1.fast" in CheckExhaustive
    @testset "cr_expm1-exhaustive.fast" begin
        test_float_range(expm1, PureLibm.cr_expm1, pos_range)
        test_float_range(expm1, PureLibm.cr_expm1, neg_range)
    end
end
if "cr_expm1" in CheckExhaustive
    @testset "cr_expm1-exhaustive" begin
        test_float_range(expm1, PureLibm.cr_expm1, pos_range, bigfloat=true)
        test_float_range(expm1, PureLibm.cr_expm1, neg_range, bigfloat=true)
    end
end

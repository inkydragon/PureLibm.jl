...
if "cr_rsqrt.fast" in CheckExhaustive
    @testset "cr_rsqrt-exhaustive.fast" begin
        test_float_range(x->1/sqrt(x), PureLibm.cr_rsqrt, lo=pos_range.lo, hi=pos_range.hi)
    end
end
if "cr_rsqrt" in CheckExhaustive
    @testset "cr_rsqrt-exhaustive" begin
        test_float_range(x->1/sqrt(x), PureLibm.cr_rsqrt, lo=pos_range.lo, hi=pos_range.hi, bigfloat=true)
    end
end

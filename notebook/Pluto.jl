...
# ╔═╡ 8793f1fc-d42c-4fe7-9839-7f29d5aa447a
using OpenLibm_jll

# ╔═╡ e5de6105-a0d5-4d11-869e-c4945f03bdec
using Plots

# ╔═╡ d115bb40-80ab-11ef-34a9-53f05a9ebc74
md"# Julia libm function test"

# ╔═╡ ca2e8d24-4881-47f6-a92b-ef9a7e2ffb08
function rel_ulp(func::Function, x::T) where T <: Union{Float32, Float64}
    y = func(x)
    Y = func(BigFloat(x))
    ulp_Y = eps(T(abs(Y)))
    rel_err = T(y - Y) / ulp_Y
...
function sample_2pi(n_sample::Int=10_000)
    min_x = 0
    max_x = 2pi
...
function plot_ulps(func::Function, x::Vector{T}) where T <: Union{Float32, Float64}
    ...
    # relative error
    p2 = plot([(min_x,max_y), (max_x, max_y)], label=nothing)  # max ULP
    plot!([(min_x,min_y), (max_x, min_y)], label=nothing)  # min ULP
    title!("$f_name")
    xaxis!("x", minorgrid = true)
    yaxis!("relative error", minorgrid = true)
    ylims!(-y_lim, y_lim)
    scatter!(
        x, y,
        ms = 0.5,
    ...
    # savefig("T:/$f_name-rel_err")

    # histogram
    p3 = histogram(y, label=nothing)
    vline!([min_y, max_y], label=nothing)
    title!("$f_name")
    xaxis!("relative error", minorgrid = true)
    yaxis!("count", minorgrid = true)
    xlims!(-y_lim, y_lim)
    # savefig("T:/$f_name-histogram")
...
plot_ulps(sinh, 
	range(-21.0, 21.0, length=10_000)
)

# ╔═╡ e706b8d4-d3cf-430a-bb3b-fb58abe1f7ef
plot_ulps(tanh, 
	range(-3, 3, length=100_000)
)

# ╔═╡ f3887fc0-8ea7-4b94-b296-911d76c080a2
plot_ulps(acosh, 
	range(1.0, 2.0, length=100_000)
)

# ╔═╡ acaf1063-c3cb-45da-b527-737d2f2d8170
plot_ulps(asinh, 
	range(-4.0, 4.0, length=100_000)
)

# ╔═╡ 5401c123-7a12-4106-96d0-bc89cb7720e0
plot_ulps(atanh, 
	range(-0.999, 0.999, length=100_000)
)

# ╔═╡ 7eb93d41-cf17-40be-9a9b-c92600407e58
md"""
...
# ╔═╡ d3f2127e-00ab-4620-982f-c1929cb67ca7
plot_ulps(exp, 
	range(-5, 10, length=100_000)
)

# ╔═╡ b9ea9a14-87da-4673-9b73-5aea9993980c
plot_ulps(exp2, 
	range(-8, 16, length=100_000)
)

# ╔═╡ 77fcfc20-616c-4c57-935a-3cd4767fbc34
plot_ulps(expm1, 
	range(-3, 0, length=100_000)
)

# ╔═╡ bb23f3fc-6f5c-4b73-9fd8-26edc75027f3
# SKIP： frexp/ilogb/ldexp

# ╔═╡ 470fea88-c068-4602-b5cc-5173dbf60b00
plot_ulps(log, 
	range(1.0, 1.2, length=100_000)
)

# ╔═╡ b17abfb5-869e-45c4-964b-04d08cdf5b53
plot(log10)

# ╔═╡ fd622a67-bffc-430a-992d-d3b5e031b4fa
plot_ulps(log10, 
	range(1.0, 1.2, length=100_000)
)

# ╔═╡ 13d5896e-a9a8-4d30-a878-2ba078b4c9dd
plot(log1p)

# ╔═╡ 0ad212cc-054a-4fd1-8f13-e39b07639a81
plot_ulps(log1p, 
	range(0.05, 0.15, length=100_000)
)

# ╔═╡ 9e3a8918-d1ba-4b9b-8eec-0ed0dc035a23
plot(log2)

# ╔═╡ ad8185ce-23a4-4f88-b559-48f7293225e2
plot_ulps(log1p, 
	range(0.05, 0.25, length=100_000)
)
...
manifest_format = "2.0"
project_hash = "f943572e2599f88d9eef348b15da1f2438d564ea"
